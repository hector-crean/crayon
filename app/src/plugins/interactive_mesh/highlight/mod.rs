...
use bevy::{picking::{focus::PickingInteraction, PickSet}, prelude::*};
    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_plugins(HighlightPlugin::<StandardMaterial> {
            highlighting_default: |mut assets| GlobalHighlight {
                hovered: MeshMaterial3d(assets.add(Color::srgb(0.35, 0.35, 0.35))),
                pressed: MeshMaterial3d(assets.add(Color::srgb(0.35, 0.75, 0.35))),
                selected: MeshMaterial3d(assets.add(Color::srgb(0.35, 0.35, 0.75))),
            },
        });
...
/// highlighting states.
pub struct HighlightPlugin<T: 'static + Material + Sync + Send> {
...
pub struct InitialHighlight<T: Material> {
    /// A handle for the initial asset state of the highlightable entity.
    pub initial: MeshMaterial3d<T>,
}
...
impl<T: Material> GlobalHighlight<T> {
    ...
    pub fn hovered(&self, h_override: &Option<&Highlight<T>>) -> Handle<T> {
        h_override
            ...
            .unwrap_or_else(|| self.hovered.0.clone())
    }
    ...
    pub fn pressed(&self, h_override: &Option<&Highlight<T>>) -> Handle<T> {
        let handle = h_override
            .and_then(|h| h.pressed.as_ref())
            .and_then(|h| h.get_handle())
            .unwrap_or_else(|| self.pressed.0.clone());
        handle
    }
...
impl<T: Material> HighlightKind<T> {
...
pub fn get_initial_highlight_asset<T: Material>(
    mut commands: Commands,
    entity_asset_query: Query<(Entity, &MeshMaterial3d<T>), Added<PickHighlight>>,
...
pub fn update_highlight_assets<T: Material>(
    global_defaults: Res<GlobalHighlight<T>>,
    mut interaction_query: Query<
        (
            &mut MeshMaterial3d<T>,
            &PickingInteraction,
            &InitialHighlight<T>,
            Option<&Highlight<T>>,
        ),
    ...
    for (mut asset, interaction, init_highlight, h_override) in &mut interaction_query {
        ...
        *asset = match interaction {
            PickingInteraction::Pressed => MeshMaterial3d(global_defaults.pressed(&h_override)),
            PickingInteraction::Hovered => MeshMaterial3d(global_defaults.hovered(&h_override)),
...
pub fn update_selection<T: Material>(
    global_defaults: Res<GlobalHighlight<T>>,
    mut interaction_query: Query<
        (
            &mut MeshMaterial3d<T>,
        ...
        Or<(Changed<Selectable>, Changed<PickingInteraction>)>,
    >,
) {
    for (mut asset, interaction, selection, init_highlight, h_override) in &mut interaction_query {
        if let PickingInteraction::None = interaction {
            *asset = if selection.is_selected {
                MeshMaterial3d(global_defaults.selected(&h_override))
            } else {
