use crate::{material::PolylineMaterialHandle, prelude::PolylineMaterial};
use bevy::{
    ecs::world::Command, ecs::{
        query::ROQueryItem,
        system::{
...
#[derive(Default)]
pub struct PolylineSpawnCommand {
...
impl PolylineSpawnCommand {
    ...
    pub fn with_material(mut self, material: PolylineMaterial) -> Self {
...
impl Command for PolylineSpawnCommand {
    fn apply(self, world: &mut World) {
        ...
        let material = match self.material {
            ...
            None => materials.add(PolylineMaterial::default()),
        };

        let polyline = world
            .resource_mut::<Assets<Polyline>>()
            .add(Polyline { vertices: self.vertices });
...
pub trait PolylineSpawner {
    // define a method that we will be able to call on `commands`
    fn spawn_polyline(&mut self, command: PolylineSpawnCommand);
}

// implement our trait for Bevy's `Commands`
impl PolylineSpawner for Commands<'_, '_> {
    fn spawn_polyline(&mut self, command: PolylineSpawnCommand) {
        self.queue(command);
